...
type Options = {
  ...
  endpointLimiters?: EndpointMatcher[];
...
function endpointConfigToRateLimiterConfig(config: EndpointRateLimitConfig): RateLimiterConfig {
  // Convert endpoint config to sliding window with burst support
  // If burst is specified, we use token bucket for better burst handling
  if (config.burst) {
    return {
      type: "tokenBucket",
      refillRate: config.limit,
      interval: config.window,
      maxTokens: config.limit + config.burst,
    };
  }
...
function matchEndpoint(
  method: string,
  path: string,
  endpointMatchers: EndpointMatcher[]
): EndpointMatcher | undefined {
  for (const matcher of endpointMatchers) {
    // Check method if specified
    if (matcher.method && matcher.method.toUpperCase() !== method.toUpperCase()) {
      continue;
    }
...
// Returns an Express middleware that rate limits per-endpoint using the Bearer token in the Authorization header
export function perEndpointRateLimitMiddleware({
  ...
  return async (req: ExpressRequest, res: ExpressResponse, next: NextFunction) => {
    if (log.requests) {
      logger.info(`RateLimiter (${keyPrefix}): request to ${req.method} ${req.path}`);
    }
    ...
    // Check if this endpoint has a specific rate limit configuration
    const endpointMatch = matchEndpoint(req.method, path, endpointLimiters);
    let limiterConfig: RateLimiterConfig;
    let rateLimitKey: string;

    if (endpointMatch) {
      // Use endpoint-specific configuration
      limiterConfig = endpointConfigToRateLimiterConfig(endpointMatch.config);
      // Include method and path pattern in the key for endpoint-specific limiting
      const patternKey =
        typeof endpointMatch.pattern === "string"
          ? endpointMatch.pattern
          : endpointMatch.pattern.source;
      rateLimitKey = `${req.method}:${patternKey}:${hashedAuthorizationValue}`;

      if (log.limiter) {
        logger.info(`RateLimiter (${keyPrefix}): using endpoint-specific config`, {
          method: req.method,
          path,
          pattern: patternKey,
      ...
      // Use default configuration with org override support
      limiterConfig = await resolveLimitConfig(
        authorizationValue,
      ...
      rateLimitKey = hashedAuthorizationValue;
    }
    ...
    res.set("retry-after", Math.ceil(secondsUntilReset).toString());
      ...
      JSON.stringify(
        {
          ...
          detail: `Rate limit exceeded for ${req.method} ${req.path}. ${$remaining}/${limit} requests remaining. Retry in ${Math.ceil(secondsUntilReset)} seconds.`,
          reset,
          limit,
          remaining: $remaining,
          retryAfter: Math.ceil(secondsUntilReset),
          error: `Rate limit exceeded for ${req.method} ${req.path}. ${$remaining}/${limit} requests remaining. Retry in ${Math.ceil(secondsUntilReset)} seconds.`,
        },
        null,
        2
      )
